...
class PandaDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if "images" not in self.data.columns:
            img_path_list = pd.NA
        else:
            img_path_list = self.data.loc[index, "images"]

        if pd.isna(img_path_list) or (isinstance(img_path_list, float) and pd.isna(img_path_list)):
            is_nan = True
        else:
            is_nan = False
        ...
        if is_nan:
            ...
            elif isinstance(img_path_list, list):
                img_path = img_path_list[0]
        else:
            if isinstance(img_path_list, str):
...
def load_examples(tokenizer, wandb_config, evaluate=False, test=False, data_dir=JSONL_DATA_DIR, img_dir=IMG_DATA_DIR):
    ...
    if os.path.isdir(path):
        ...
        if path.endswith('.csv'):
            ...
            final_df = pd.read_json(path)
        else:
            raise ValueError("Unsupported file format. Must be .csv or .json")
        final_df["data_src"] = os.path.basename(path)

    if 'prob' not in final_df.columns:
        final_df['prob'] = 1.0
