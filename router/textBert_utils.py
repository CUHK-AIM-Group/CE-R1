...
def tokenize_and_encode_data(sentences_iterable, tokenizer_encoder, max_sent_len, labels_iterable, multiclass):
    ...
    for sent, label in zip(sentences_iterable, labels_iterable):
        ...
        bs,ch= encoded_dict['input_ids'].shape
        if ch != 300:
            print("encoded_dict['input_ids']:", ch)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % wandb_config.gradient_accumulation_steps == 0:
                ...
                current_model_is_best = False
                if wandb_config.logging_steps > 0 and global_step % wandb_config.logging_steps == 0:
                    logs = {}
                    if wandb_config.evaluate_during_training and global_step % wandb_config.val_steps == 0: 
                        ...
                        logger.info("Epoch: {}, Steps: {}, Current Accuracy :{}".format(epoch, global_step, current_acc))

                    if current_acc > best_acc:
                        best_acc = current_acc
                        logger.info("Epoch: {}, Steps: {}, Best Accuracy :{}".format(epoch, global_step, best_acc))
                        current_model_is_best = True
                ...
                if wandb_config.save_steps > 0 and global_step % wandb_config.save_steps == 0 or current_model_is_best:
